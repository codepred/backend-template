...
import com.codepred.cleanrepo.account.command.SetPasswordCommand;
import com.codepred.cleanrepo.account.command.UserRegisterCommand;
import com.codepred.cleanrepo.account.dto.*;
import com.codepred.cleanrepo.account.command.RegisterCommand;
import com.codepred.cleanrepo.account.command.HostRegisterCommand;
...
class AccountController {
    ...
    @CrossOrigin(origins = {"http://20.215.224.244", "http://localhost:5173"})
    ResponseEntity<Object> login(@RequestBody RegisterCommand.Json requestJson) {
        ...
        Optional<Jwt> jwt = jwtService.generateJwt(new AccountCredentials(account.get().getAccountId(),
                                                                          // TODO tu bedzie mapper
                                                                          account.get().getEmail(),
                                                                          account.get().getEncodedPassword(),
                                                                          account.get().getRole(),
                                                                          account.get().getLocked()));
        String token = jwt.orElseThrow().jwt(); // co zwracac jak jwt jest puste?
    ...
    ResponseEntity<Object> registerHost(@RequestHeader("Authorization") String bearerToken,
        ...
        String link = "http://20.215.224.244/set-password?token=" + randomUUID.toString().replaceAll("_", "") + "&email="
            + account.getEmail().getEmail();
        emailService.sendEmail(account.getEmail().getEmail(), "codepred.test@gmail.com", link);
    ...
    ResponseEntity<Object> createSurvey(@RequestHeader("Authorization") String bearerToken, @RequestBody SurveyDataDto survey) {
        ...
        Set<String> set = new HashSet<String>(questions);
    ...
    ResponseEntity<Object> submitSurvey(@RequestHeader("Authorization") String bearerToken,
        ...
        submittedSurvey.object()
            .setSurvey(surveyWithChosenQuestions.get()); // zastanowic sie czy tu bedzie survey czy surveywithchosenquestions
    ...
    @CrossOrigin(origins = {"http://20.215.224.244", "http://localhost:5173"})
    ResponseEntity<Object> getSurveyList(@RequestHeader("Authorization") String bearerToken) { // bedzie walidowane dla roznych rol
        Account user;
        try {
            user = accountService.getAccountFromToken(bearerToken);
        } catch (InvalidJwtException e) {
                ...
                for (SubmittedSurvey survey : surveyWithChosenQuestions.getSubmittedSurveys()) {
                    if (survey.getVerifier() != null && user.getAccountId() == survey.getVerifier().getAccountId()) {
                        surveyForSurveyListDtos.add(new SurveyForSurveyListDto(survey.getSubmittedSurveyId(),
                                                                               ...
                                                                               surveyWithChosenQuestions.getAssignedDate(),
                                                                               // na razie null, bo nie wiadomo kiedy sie przypisuje
                                                                               survey.getStatus()));
                        continue;
                    }
    ...
    @GetMapping("/admin/get-survey-results/{survey-id}/{company-id}") // do przemyslenia ale wydaje sie ze dziala
    @CrossOrigin(origins = {"http://20.215.224.244", "http://localhost:5173"})
    ...
    ResponseEntity<Object> generateInvoice(@RequestHeader("Authorization") String bearerToken,
                                           @PathVariable("survey-id") int surveyId,
                                           @PathVariable("company-id") int hostId)
        throws IOException, CsvRequiredFieldEmptyException, CsvDataTypeMismatchException, DocumentException {
        Account admin;
        try {
            admin = accountService.getAccountFromToken(bearerToken);
        } catch (InvalidJwtException e) {
        ...
        try {
            ...
            // Convert yourObject to JSON
            String jsonData = gson.toJson(resultDto);

            // Create ByteArrayResource from JSON data
    ...
    public ResponseEntity<Object> downloadResultsCsv(@RequestHeader("Authorization") String bearerToken,
        ...
        try {
                ...
                for (ResultAnswerDto answer : question.answers()) {
                    ...
                    if (resultDto.score() != null) {
                        try {
                            ...
                            // Handle the case where resultDto.score() is not a valid integer
                            scoreAsString = ""; // or any default value you prefer
                        }
                    }
            ...
            // Create ByteArrayResource from CSV data
            ByteArrayResource resource = new ByteArrayResource(csvContent.toString().getBytes());

            // Prepare response headers
            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.TEXT_PLAIN); // set content type as CSV
